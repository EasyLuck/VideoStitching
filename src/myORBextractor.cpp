...
// _nfeatures:特征点数量
// _scaleFactor:相邻图像层比例系数
// _nlevels：金字塔层数
// _iniThFAST：检测Fast角点阈值
// _minThFAST:没有检测到角点的前提下降低阈值
ORBextractor::ORBextractor(int _nfeatures, float _scaleFactor, int _nlevels,
         int _iniThFAST, int _minThFAST):
    nfeatures(_nfeatures), scaleFactor(_scaleFactor), nlevels(_nlevels),
    iniThFAST(_iniThFAST), minThFAST(_minThFAST)
{
    mvScaleFactor.resize(nlevels); //每层相对于原始图像的缩放系数，第一层为1.0
    ...
    float factor = 1.0f / scaleFactor; //尺度因子的倒数
    ...
    //   主要是将每层的特征点数量进行均匀控制
    float nDesiredFeaturesPerScale = nfeatures*(1 - factor)/(1 - (float)pow((double)factor, (double)nlevels));//第一层特征点数量
    ...
    //每层都是四舍五入的特征点,加起来的总数有可能比设定的总数要多，这样最上一层就设为0
    mnFeaturesPerLevel[nlevels-1] = std::max(nfeatures - sumFeatures, 0);
 
    const int npoints = 512;
    const Point* pattern0 = (const Point*)bit_pattern_31_;
 
    //标准库提供的back_inserter模板函数很方便，因为它为container返回一个back_insert_iterator迭代器，这样，复制的元素都被追加到container的末尾了
    std::copy(pattern0, pattern0 + npoints, std::back_inserter(pattern));
 
    //This is for orientation  计算特征点方向
