...
cv::Mat imageStitching::imageProcess(Mat img1, Mat img2)
{
    ...
    Mat img2ReSize(img2.rows, img1.cols+img2.cols, CV_8UC3);
    img2.copyTo(img2ReSize(Rect(img1.cols, 0, img2.cols, img2.rows)));
    cvtColor(img2ReSize, grayImage2, CV_RGB2GRAY);
    // //提取特征点    
    // Ptr<FeatureDetector> Detector = ORB::create();
    // vector<KeyPoint> keyPoint11, keyPoint21;
    //     //特征点描述，为下边的特征点匹配做准备    
    // Ptr<DescriptorExtractor> Descriptor = ORB::create();
    // Mat imageDesc11, imageDesc21;

    // chrono::steady_clock::time_point t1  = chrono::steady_clock::now();
    
    // Detector->detect(grayImage1, keyPoint11);
    // Detector->detect(grayImage2, keyPoint21);
    // Descriptor->compute(grayImage1, keyPoint11, imageDesc11);
    // Descriptor->compute(grayImage2, keyPoint21, imageDesc21);

    
    // chrono::steady_clock::time_point t2 = chrono::steady_clock::now();
    // chrono::duration<double> time_used = chrono::duration_cast<chrono::duration<double>>(t2-t1);
    // cout << "traditional ORB cost : " << time_used.count() << " seconds." << endl;
    // cout << "traditional ORB keyPoint : " << keyPoint11.size()  << endl;
    ...
    // cout << "ORBextractor cost : " << time_used.count() << " seconds." << endl;
    // cout << "ORBextractor keyPoint : " << keyPoint1.size()  << endl;
    ...
    Mat homo;
    Mat dst(img2.rows, img1.cols+img2.cols, CV_8UC3);
    dst.setTo(0);

    // Stitcher sitcher = Stitcher::createDefault();
    // sitcher.stitch(homo,dst);
    ...
    //计算配准图的四个顶点坐标
        ...
        //图像配准  
        Mat  imageTransform2;
        // Mat imageTransform1(img1.rows, img1.cols+img2.cols, CV_8UC3);
    /*
    ...
    */
        ...
        warpPerspective(img1, imageTransform2, homo, Size(img1.cols+img2.cols, img1.rows));
